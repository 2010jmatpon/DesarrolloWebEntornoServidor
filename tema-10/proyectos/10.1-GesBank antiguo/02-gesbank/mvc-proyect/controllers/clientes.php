...
class Clientes extends Controller
{
    ...
    public function exportCSV($param = [])
    {
        ...
        if (!isset($_SESSION['id'])) {
            ...
            $nombre = "cliente_" . $cliente->nombre . "_" . $cliente->apellidos . ".csv";
            ...
            fputcsv($archivo, ['ID', 'Apellidos', 'Nombre', 'Teléfono', 'Ciudad', 'DNI', 'Email']);
    ...
    # Método importCSV
    # Permite importar nuevos registros de clientes desde un archivo CSV
            ...
            if ($archivo == TRUE) {
                while (($fila = fgetcsv($archivo, 0, ';')) !== false) {
                    ...
                    if ($this->model->validateUniqueEmail($email) && $this->model->validateUniqueDni($dni)) {
                        ...
                        echo "Email o Dni ya existentes";
                    }
                }
                fclose($archivo);
    ...
    public function pdf($param = [])
    {
        ...
        if (!isset($_SESSION['id'])) {
            ...
            $get_clientes = $this->model->get();
            $clientes = $get_clientes->fetchAll(PDO::FETCH_ASSOC);
            $pdf = new PDFClientes();
