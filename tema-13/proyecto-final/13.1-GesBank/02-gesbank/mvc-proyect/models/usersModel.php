...
class usersModel extends Model
{
    ...
    public function update(classUser $user, $id, $rol)
    {
        try {
            ...
            $stmt->bindParam(':password', $user->password, PDO::PARAM_STR, 50);
            ...
            $stmt_rol = $conexion->prepare($updateRol);
            $stmt_rol->bindParam(':role_id', $rol);
            $stmt_rol->bindParam(':user_id', $id);
            $stmt_rol->execute();
            $stmt->execute();
            // var_dump("Actualización ejecutada con éxito.") ;
            // exit();
